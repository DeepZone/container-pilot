...
async function load() {
  ...
  $('#containerRows').innerHTML = status.containers.map((container) => {
    ...
    return `<tr><td><div class="name">${esc(container.name)}</div><small>${esc(container.id.slice(0, 12))}</small></td><td class="image">${esc(container.image)}</td><td><span class="badge ${container.state === 'running' ? 'good' : 'bad'}">${esc(container.state)}</span><br><small>${esc(container.status)}</small>${busy ? `<br><span class="badge warn">${esc(container.operation)}</span>` : ''}</td><td class="${container.scan?.error ? 'bad' : update ? 'warn' : 'good'}">${container.scan?.error ? esc(container.scan.error) : update ? 'Verfügbar' : 'Aktuell'}</td><td class="lastUpdate">${esc(lastUpdate)}</td><td>${updateMode}</td><td><label class="toggle"><input type="checkbox" data-policy="${container.id}" ${container.policy.auto ? 'checked' : ''} ${disabled}> Aktiv</label></td><td><div class="actions">${update ? `<button class="primary" data-update="${container.id}" data-name="${esc(container.name)}" ${disabled}>Update</button>` : ''}${canLatest ? `<button data-latest="${container.id}" data-name="${esc(container.name)}" ${disabled}>→ latest</button>` : ''}${rollbackButton}</div></td></tr>`;
  }).join('');
  ...
  document.querySelectorAll('[data-update]').forEach((element) => { element.onclick = () => confirmAction('Update installieren', `${element.dataset.name} wird neu erstellt und bis zur Betriebsbereitschaft geprüft. Achtung: Volumes und Datenbankmigrationen werden bei einem Rollback nicht zurückgesetzt.`, () => api(`/api/containers/${element.dataset.update}/update`, { method: 'POST', body: '{}' })); });

...
async function load() {
  ...
  $('#containerRows').innerHTML = status.containers.map((container) => {
    ...
    const rollbackButton = container.rollback ? `<button class="rollback" data-rollback="${container.id}" data-name="${esc(container.name)}" data-image="${esc(container.rollback.displayImage)}" data-created="${esc(container.rollback.createdAt)}" ${isAdmin ? '' : 'disabled'}>Rollback</button>` : '';
    return `<tr><td><div class="name">${esc(container.name)}</div><small>${esc(container.id.slice(0, 12))}</small></td><td class="image">${esc(container.image)}</td><td><span class="badge ${container.state === 'running' ? 'good' : 'bad'}">${esc(container.state)}</span><br><small>${esc(container.status)}</small></td><td class="${container.scan?.error ? 'bad' : update ? 'warn' : 'good'}">${container.scan?.error ? esc(container.scan.error) : update ? 'Verfügbar' : 'Aktuell'}</td><td class="lastUpdate">${esc(lastUpdate)}</td><td>${updateMode}</td><td><label class="toggle"><input type="checkbox" data-policy="${container.id}" ${container.policy.auto ? 'checked' : ''} ${isAdmin ? '' : 'disabled'}> Aktiv</label></td><td><div class="actions">${update ? `<button class="primary" data-update="${container.id}" data-name="${esc(container.name)}" ${isAdmin ? '' : 'disabled'}>Update</button>` : ''}${canLatest ? `<button data-latest="${container.id}" data-name="${esc(container.name)}" ${isAdmin ? '' : 'disabled'}>→ latest</button>` : ''}${rollbackButton}</div></td></tr>`;
  }).join('');
